...
class ConsoleSession : public enable_shared_from_this<ConsoleSession> {
 ...
 public:
  ConsoleSession(struct RemoteServer rs) : q(rs.host,rs.port), RS(rs){
 // 	cout << "<script>document.getElementById('s"+ to_string(RS.id) +"').innerHTML += \'"+rs.file+"\';</script>";
  	testcase.open("test_case/"+rs.file);
//	cout << "<script>document.getElementById('s"+ to_string(RS.id) +"').innerHTML += \'"+rs.file+"\';</script>";
  }
 ...
 private:
  void do_resolve(){
	...
	resolv.async_resolve(q , [this,self](boost::system::error_code ec,ip::tcp::resolver::iterator it){
		//cout << "<script>document.getElementById('s"+ to_string(RS.id) +"').innerHTML += \'hi\';</script>";
		if(!ec) do_connect(it);			
	});
  ...
  void do_read() {
    ...
    tcp_socket.async_read_some(
        ...
        [this, self](boost::system::error_code ec, size_t length) {
          	if (!ec){
		//	cout << "<script>document.getElementById('s"+ to_string(RS.id) +"').innerHTML += \'hi1\';</script>";
	  		string shell2client;
		        shell2client.append(_data.data(),_data.data()+length);
			//cout << "<script>document.getElementById('s"+ to_string(RS.id) +"').innerHTML += \'"+shell2client+"\';</script>";
			output_shell(RS.id,shell2client);
			if(shell2client.find('%')!=string::npos){
				string cmd;
